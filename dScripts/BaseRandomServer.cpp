...
void BaseRandomServer::SpawnSection(Entity* self, const std::string& sectionName, float iMultiplier) {
	...
	for (const auto& spawnerData : spawnLoad->entries) {
		...
		SetSpawnerNetwork(self, spawnerName, spawnNum, spawnerData.lot); // "Lets put a pin in that and come back to it some other night 2024/12/30 23:37 ジェーミーは楽しいなんだよ！"
	}
}
